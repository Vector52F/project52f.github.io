...
/// (26 000 000 blocks ≈ 30 days at 10 blocks/second).
///
...
mod project52f {
    ...
    /// At QF Network target of 10 blocks/second: 26 000 000 blocks ≈ 30 days.
    ///
    ...
    pub const SHIELD_END_BLOCK: u32 = 26_000_000;

    // ── GREAT DRAIN CONSTANTS ─────────────────────────────────────────────────

    /// Great Drain threshold: 520 000 000 $52F$ tokens (with 18 decimals).
    /// This is a permanent compile-time constant — immutable post-deploy.
    /// Denominated in 52F token base units (not QF). Price-independent.
    ...
    // ── SNIPER HARVEST — blocks 0 – 10 399 (≈ 17.2 min at 0.1 s/block) ───────
    //
    // Three-phase launch window that converts bot snipes into protocol liquidity.
    // 100% of tax collected in both zones routes to dampener_tax_accumulated.
    // Prize pot and team receive ZERO during this window.
    //
    // Zone A (0 – 5 199):   flat 95% tax.
    // Zone B (5 200 – 10 399): linear decay from 52% → ~3%,
    //                          dropping 1% every 104 blocks (50 steps total).
    //
    // Math guarantee:
    //   £1 000 snipe at £0.01/QF  →  100 000 QF transacted
    //   Zone A tax (95%)          →   95 000 QF donated to Dampener Vault ✓

    /// Last block (exclusive) of the Sniper Harvest window.
    /// At 10 blocks/s this window is exactly 1 040 seconds ≈ 17.3 minutes.
    pub const SNIPER_END_BLOCK:    u32 = 10_400;

    /// Last block (exclusive) of Zone A (flat 95% tax).
    pub const ZONE_A_END_BLOCK:    u32 = 5_200;

    /// Zone A flat tax in BPS (95%).
    pub const ZONE_A_TAX_BPS:      u128 = 9_500;

    /// Zone B opening tax in BPS (52% — the 52F signature).
    pub const ZONE_B_START_BPS:    u128 = 5_200;

    /// BPS reduction per step in Zone B (1% = 100 BPS).
    pub const ZONE_B_STEP_BPS:     u128 = 100;

    /// Blocks per Zone B step (1% drop every 104 blocks).
    pub const ZONE_B_STEP_BLOCKS:  u32  = 104;
    ...
    /// Minimum valid transaction size in normal operation (1 token, 18 decimals).
    pub const MIN_TRANSACTION_THRESHOLD: u128 = 1_000_000_000_000_000_000;

    /// Minimum transaction size during Zone A (blocks 0 – ZONE_A_END_BLOCK).
    /// Raised to 1 000 tokens to prevent dust-spam during the 95% flat-tax window.
    /// Reverts to MIN_TRANSACTION_THRESHOLD from Zone B onwards.
    pub const SNIPER_MIN_THRESHOLD: u128 = 1_000_u128 * 1_000_000_000_000_000_000_u128;

    // ── COLLISION ENGINE GATES ────────────────────────────────────────────────

    /// Minimum $52F$ balance a caller must HOLD (after their buy is credited) to
    /// earn a collision slot in the Sequencer Satellite epoch.
    /// 52 000 000 tokens × 10^18 = skin-in-the-game gate.
    pub const HOLDING_GATE: u128 = 52_000_000_u128 * 1_000_000_000_000_000_000_u128;

    /// Minimum buy amount (in $52F$ base units) required to qualify for a
    /// collision slot.  Equal to the holding gate — you must buy at least as
    /// many tokens as the gate in one transaction to enter the epoch.
    pub const COLLISION_MIN_BUY: u128 = 52_000_000_u128 * 1_000_000_000_000_000_000_u128;
    ...
    pub struct Project52F {
        ...
        // ── Launch gate ───────────────────────────────────────────────────
        /// One-way trading gate — starts false, owner calls enable_trading()
        /// once all liquidity is deployed. Can never be set back to false.
        trading_enabled: bool,
        /// Whether the Dampener is whitelisted for pre-trading transfers.
        transfer_whitelist_dampener: bool,
        /// DEX router whitelisted for pre-trading transfers.
        transfer_whitelist_dex_router: Option<Address>,

        // ── Safety ───────────────────────────────────────────────────────
        paused: bool,
        /// Reentrancy guard — set during buy() and sell() execution.
        is_executing: bool,
    }
    ...
    /// Emitted when the Sequencer Satellite pulls the accumulated team tax.
    #[ink(event)]
    pub struct TeamTaxPulled {
        #[ink(topic)]
        recipient: Address,
        amount: U256,
        epoch_id: u32,
    ...
    /// Emitted once when the owner permanently enables public trading.
    #[ink(event)]
    pub struct TradingEnabled {
        at_block: u32,
    }

    /// Emitted when the owner pre-seeds 52F to the Dampener before trading opens.
    #[ink(event)]
    pub struct SeedTransferred {
        #[ink(topic)]
        recipient: Address,
        amount: U256,
    }
    ...
    pub enum Error {
        ...
        /// Contract is paused.
        ContractPaused,
        /// Re-entrant call detected — buy/sell cannot be re-entered.
        Reentrant,
        /// Trading is not yet enabled — market has not launched.
        TradingNotEnabled,
        /// Trading is already enabled — cannot call enable_trading() again.
        TradingAlreadyEnabled,
        /// Required contract address is not configured.
        WiringIncomplete,
        /// Team tax accumulator is empty — nothing to pull.
        TeamTaxEmpty,
    }
    ...
    impl Project52F {
        ...
        /// The drain threshold is a permanent compile-time constant — DEFAULT_DRAIN_THRESHOLD.
        /// Deploy the Token Engine.
        ///
        /// `vault_seed` is the QF pre-seeded directly into `dampener_tax_accumulated`
        /// at construction — the 10 400 QF Dampener Vault buffer from the 80/20
        /// capital split (41 600 QF → DEX LP, 10 400 QF → here).
        /// Pass `U256::ZERO` for testnet deploys with no seed capital.
        #[ink(constructor)]
        pub fn new(initial_supply: U256, name: String, symbol: String, vault_seed: U256) -> Self {
            ...
            Self {
                ...
                dampener_tax_accumulated: vault_seed,   // 10 400 QF pre-seeded at deploy
                lp_pair_token_reserve: U256::ZERO,
                drain_event_count: 0,
                trading_enabled: false,
                transfer_whitelist_dampener: false,
                transfer_whitelist_dex_router: None,
                trading_enabled: false,
                transfer_whitelist_dampener: false,
                transfer_whitelist_dex_router: None,
                paused: false,
        ...
        /// Process a buy transaction.  Tax routing is fully dynamic:
        /// Sniper Harvest → Hardening → Scarcity (see `route_tax`).
        #[ink(message)]
        pub fn buy(&mut self, amount: U256) -> Result<(), Error> {
            self.assert_not_paused()?;
            if !self.trading_enabled { return Err(Error::TradingNotEnabled); }
            if self.is_executing { return Err(Error::Reentrant); }
            self.is_executing = true;
            let result = self.buy_inner(amount);
            self.is_executing = false;
            result
        }

        fn buy_inner(&mut self, amount: U256) -> Result<(), Error> {
            self.assert_above_threshold(amount)?;
            let caller = self.env().caller();
            let (dampener, prize, team, net) = self.route_tax(amount, false)?;
            self.debit_balance(caller, amount)?;
            self.apply_tax(dampener, prize, team)?;
            self.credit_balance(caller, net)?;
            self.env().emit_event(Transfer {
                from: Some(caller),
                to: Some(self.env().account_id()),
                value: amount.saturating_sub(net),
            });
            self.check_and_execute_drain()?;

            // ── Collision Engine Gate ─────────────────────────────────────
            // Register the buyer for the current epoch only if:
            //   1. Buy amount ≥ COLLISION_MIN_BUY (52M tokens)
            //   2. Post-buy balance ≥ HOLDING_GATE (52M tokens)
            // Both gates checked here; Sequencer trusts the Token Engine.
            if amount >= U256::from(COLLISION_MIN_BUY) {
                let post_buy_balance = self.balances.get(caller).unwrap_or(U256::ZERO);
                if post_buy_balance >= U256::from(HOLDING_GATE) {
                    // Best-effort XCC — if Sequencer is unregistered or epoch
                    // is full, we silently skip rather than reverting the buy.
                    let _ = self.try_register_with_sequencer(caller);
                }
            }
        ...
        /// Process a sell transaction.  Same dynamic routing as `buy`.
        #[ink(message)]
        pub fn sell(&mut self, amount: U256) -> Result<(), Error> {
            ...
            if self.is_executing { return Err(Error::Reentrant); }
            self.is_executing = true;
            let result = self.sell_inner(amount);
            self.is_executing = false;
            result
        }

        fn sell_inner(&mut self, amount: U256) -> Result<(), Error> {
            self.assert_above_threshold(amount)?;
            let caller = self.env().caller();
            let (dampener, prize, team, net) = self.route_tax(amount, true)?;
            self.debit_balance(caller, amount)?;
            self.apply_tax(dampener, prize, team)?;
            self.credit_balance(caller, net)?;
            self.env().emit_event(Transfer {
        ...
        fn check_and_execute_drain(&mut self) -> Result<(), Error> {
            let drain_threshold = U256::from(DEFAULT_DRAIN_THRESHOLD);
            if self.prize_pot_accumulated < drain_threshold {
                return Ok(());
            }
            ...
            if hardening && self.dampener_address.is_some() {
                // Hardening with Dampener registered: queue for LP pairing.
                self.lp_pair_token_reserve = self
                    .lp_pair_token_reserve
                    .checked_add(paired_52f)
                    .ok_or(Error::Overflow)?;
            } else {
                // Scarcity, OR Hardening with no Dampener registered:
                // Double Burn — paired_52f is also sent to DEAD_ADDRESS.
        ...
        #[ink(message)]
        /// Pull the epoch prize yield to the Sequencer Satellite for distribution.
        ///
        /// Implements the Fibonacci 34/13/5 split at source:
        ///
        /// ```text
        ///   pot_cut  = prize_pot × 13/52  — stays in pot forever (Great Drain fuel)
        ///   sendable = prize_pot × 39/52  — transferred to Sequencer
        /// ```
        ///
        /// The Sequencer then splits `sendable` as 34/39 to winners and 5/39 to
        /// liquidity, which together with the 13/52 retained here gives the full
        /// 34/13/5 distribution across the epoch.
        ///
        /// **The 13/52 pot_cut is never extracted.**  It silently accumulates in
        /// `prize_pot_accumulated` until it crosses `prize_drain_threshold`
        /// (520 000 000 $52F$ token equivalent), at which point `check_and_execute_drain`
        /// fires the Great Drain automatically on the next buy or sell transaction.
        ///
        /// **Caller:** Must be the registered `sequencer_satellite`.
        pub fn pull_prize_tax(&mut self) -> Result<U256, Error> {
            ...
            // ── Fibonacci retention: 13/52 stays, 39/52 is sent ──────────
            // Using integer arithmetic: pot_cut = total_pot × 13 / 52
            // sendable = total_pot - pot_cut  (avoids second division rounding)
            let pot_cut = total_pot
                .checked_mul(U256::from(13u8))
                .ok_or(Error::Overflow)?
                .checked_div(U256::from(52u8))
                .ok_or(Error::Overflow)?;

            let sendable = total_pot.saturating_sub(pot_cut);

            // ── State update BEFORE external call ─────────────────────────
            // pot retains the 13/52 cut; only sendable leaves.
            self.prize_pot_accumulated = pot_cut;
            self.env()
                .transfer(satellite, sendable)
                .map_err(|_| Error::TransferFailed)?;

            self.env().emit_event(PrizePotPulled {
                epoch_id: self.drain_event_count,
                burn_amount: U256::ZERO,   // no burn here — Great Drain handles burns
                yield_amount: sendable,
                satellite,
            });
        ...
        pub fn transfer(&mut self, to: Address, value: U256) -> Result<(), Error> {
            ...
            if !self.trading_enabled && !self.is_transfer_whitelisted(from) {
                return Err(Error::TradingNotEnabled);
            }
        ...
        pub fn transfer_from(
            ...
            if !self.trading_enabled
                && !self.is_transfer_whitelisted(from)
                && !self.is_transfer_whitelisted(caller)
            {
                return Err(Error::TradingNotEnabled);
            }
        ...
        #[ink(message)]
        pub fn get_drain_threshold(&self) -> U256 { U256::from(DEFAULT_DRAIN_THRESHOLD) }
        ...
        /// Returns `(buy_tax_bps, sell_tax_bps, dampener_bps)` for `current_block`.
        ///
        /// During the Sniper Harvest (elapsed < `SNIPER_END_BLOCK`) buy and sell
        /// carry the same rate and dampener receives 100% of it (prize = team = 0).
        ///
        /// | Zone / Phase      | buy_tax | sell_tax | dampener |
        /// |-------------------|---------|----------|----------|
        /// | Zone A  (0–5199)  | 9 500   | 9 500    | 9 500    |
        /// | Zone B  (5200–10399) | linear | linear | = total  |
        /// | Hardening         | 272     | 314      | 125      |
        /// | Scarcity          | 272     | 314      | 50       |
        #[ink(message)]
        pub fn get_tax_rates(&self, current_block: u32) -> (u32, u32, u32) {
            let elapsed = current_block.saturating_sub(self.deploy_block);
            if elapsed < SNIPER_END_BLOCK {
                let bps = Self::sniper_tax_bps(elapsed) as u32;
                return (bps, bps, bps); // 100% of tax → dampener
            }
            let dampener = if elapsed < SHIELD_END_BLOCK {
                TAX_DAMPENER_SHIELD_BPS
            } else {
                TAX_DAMPENER_BASE_BPS
            } as u32;
            (E_BUY_TAX_BPS as u32, PI_SELL_TAX_BPS as u32, dampener)
        }
        ...
        /// Permanently enable public trading.
        ///
        /// One-way toggle — can never be reversed.  Call this only after:
        ///   1. verify_wiring() returns (true, true, true)
        ///   2. Dampener has called deploy_initial_liquidity() successfully
        ///   3. Tiny seed add is confirmed in the DEX pool
        ///
        /// Once called, buy() and sell() open to the public and the Sniper
        /// Harvest window begins.
        #[ink(message)]
        pub fn enable_trading(&mut self) -> Result<(), Error> {
            self.only_owner()?;
            if self.trading_enabled {
                return Err(Error::TradingAlreadyEnabled);
            }
        ...
        /// Register the DEX router address for pre-trading transfer whitelist.
        ///
        /// Must be called before deploy_initial_liquidity() on the Dampener.
        /// After trading is enabled this has no effect on transfer permissions
        /// (all transfers are unrestricted once trading opens).
        #[ink(message)]
        pub fn set_dex_router_whitelist(&mut self, addr: Address) -> Result<(), Error> {
            self.only_owner()?;
            self.transfer_whitelist_dex_router = Some(addr);
            Ok(())
        }

        /// Activate the Dampener's pre-trading transfer whitelist slot.
        ///
        /// Call once after set_dampener_address() is confirmed.
        /// Allows the Dampener to receive and deploy 52F tokens pre-launch.
        #[ink(message)]
        pub fn enable_dampener_whitelist(&mut self) -> Result<(), Error> {
            self.only_owner()?;
            self.transfer_whitelist_dampener = true;
            Ok(())
        }

        /// Transfer 52F tokens to the Dampener before trading opens.
        ///
        /// Owner-only.  Bypasses the trading gate entirely so the Dampener
        /// can receive the bulk 52F supply and deploy initial liquidity
        /// before the market opens.
        ///
        /// This is the only pre-trading transfer that does NOT require the
        /// recipient to be whitelisted — owner is always trusted for seeding.
        #[ink(message)]
        pub fn seed_transfer(&mut self, to: Address, amount: U256) -> Result<(), Error> {
            self.only_owner()?;
            self.assert_not_paused()?;
            self.transfer_impl(self.owner, to, amount)?;
            self.env().emit_event(SeedTransferred { recipient: to, amount });
            Ok(())
        }
        ...
        /// Pull the accumulated team tax and transfer directly to the owner.
        ///
        /// **Caller:** Must be the registered Sequencer Satellite.
        /// Called automatically at each epoch flush — no manual action needed.
        /// Returns zero silently if nothing has accumulated yet (Hardening phase
        /// has no team tax, so this is a no-op for the first ~30 days).
        #[ink(message)]
        pub fn pull_team_tax(&mut self) -> Result<U256, Error> {
            self.assert_not_paused()?;

            let caller = self.env().caller();
            let satellite = self.sequencer_satellite
                .ok_or(Error::NoSatelliteRegistered)?;
            if caller != satellite {
                return Err(Error::NotSequencerSatellite);
            }
            ...
            if amount.is_zero() {
                return Ok(U256::ZERO); // No-op during Hardening — not an error.
            }

            // State update before transfer.
            self.team_tax_accumulated = U256::ZERO;

            let owner = self.owner;
            let epoch_id = self.drain_event_count; // reuse counter as epoch ref

            self.env()
                .transfer(owner, amount)
                .map_err(|_| Error::TransferFailed)?;

            self.env().emit_event(TeamTaxPulled {
                recipient: owner,
                amount,
                epoch_id,
            });
        ...
        /// Verify that all required cross-contract addresses are configured.
        ///
        /// Returns `(sequencer_set, dampener_set, all_ok)`.
        /// Call immediately after deployment and wiring — before announcing launch.
        /// A `false` in any slot means that mechanical subsystem is disabled.
        #[ink(message)]
        pub fn verify_wiring(&self) -> (bool, bool, bool) {
            let sequencer_set = self.sequencer_satellite.is_some();
            let dampener_set  = self.dampener_address.is_some();
            let all_ok        = sequencer_set && dampener_set;
            (sequencer_set, dampener_set, all_ok)
        }
        ...
        /// Master tax router — single source of truth for all three phases.
        ///
        /// Returns `(dampener_share, prize_share, team_share, net_amount)`.
        ///
        /// **Sniper Harvest** (`elapsed < SNIPER_END_BLOCK`):
        ///   `dampener = sniper_bps% of amount`, prize = 0, team = 0.
        ///
        /// **Hardening** (`elapsed < SHIELD_END_BLOCK`):
        ///   dampener = 1.25%, prize = 1.47%/1.89% (buy/sell), team = 0.
        ///
        /// **Scarcity** (`elapsed ≥ SHIELD_END_BLOCK`):
        ///   dampener = 0.50%, prize = 1.47%/1.89%, team = 0.75%.
        fn route_tax(
            &self,
            amount: U256,
            is_sell: bool,
            ...
            let denom  = U256::from(BPS_DENOMINATOR);
            let elapsed = self.env().block_number().saturating_sub(self.deploy_block);

            if elapsed < SNIPER_END_BLOCK {
                let bps = Self::sniper_tax_bps(elapsed);
                let tax = amount
                    .checked_mul(U256::from(bps)).ok_or(Error::Overflow)?
                    .checked_div(denom).ok_or(Error::Overflow)?;
                return Ok((tax, U256::ZERO, U256::ZERO, amount.saturating_sub(tax)));
            }

            let hardening    = elapsed < SHIELD_END_BLOCK;
            let d_bps        = if hardening { TAX_DAMPENER_SHIELD_BPS } else { TAX_DAMPENER_BASE_BPS };
            let prize_bps    = if is_sell  { SELL_TAX_PRIZE_BPS } else { BUY_TAX_PRIZE_BPS };

            let dampener = amount.checked_mul(U256::from(d_bps)).ok_or(Error::Overflow)?
                .checked_div(denom).ok_or(Error::Overflow)?;
            let prize    = amount.checked_mul(U256::from(prize_bps)).ok_or(Error::Overflow)?
                .checked_div(denom).ok_or(Error::Overflow)?;
            let team     = if hardening { U256::ZERO } else {
                amount.checked_mul(U256::from(TAX_TEAM_BPS)).ok_or(Error::Overflow)?
                    .checked_div(denom).ok_or(Error::Overflow)?
            };
            let total = dampener.checked_add(prize).ok_or(Error::Overflow)?
                .checked_add(team).ok_or(Error::Overflow)?;
            Ok((dampener, prize, team, amount.saturating_sub(total)))
        }

        /// Accumulate the three tax shares into their respective storage slots.
        fn apply_tax(&mut self, dampener: U256, prize: U256, team: U256) -> Result<(), Error> {
            self.dampener_tax_accumulated = self.dampener_tax_accumulated
                .checked_add(dampener).ok_or(Error::Overflow)?;
            self.prize_pot_accumulated = self.prize_pot_accumulated
                .checked_add(prize).ok_or(Error::Overflow)?;
            if !team.is_zero() {
        ...
        /// Pure function: sniper tax in BPS for `elapsed` < `SNIPER_END_BLOCK`.
        ///
        /// Zone A (0–5 199):   `ZONE_A_TAX_BPS` = 9 500 (flat 95%).
        /// Zone B (5 200–10 399): `ZONE_B_START_BPS − (steps × ZONE_B_STEP_BPS)`
        ///   where `steps = (elapsed − ZONE_A_END_BLOCK) / ZONE_B_STEP_BLOCKS`.
        ///   Maximum 50 steps → floor of 3% at block 10 296.
        fn sniper_tax_bps(elapsed: u32) -> u128 {
            if elapsed < ZONE_A_END_BLOCK {
                ZONE_A_TAX_BPS
            } else {
                let steps = ((elapsed - ZONE_A_END_BLOCK) / ZONE_B_STEP_BLOCKS) as u128;
                ZONE_B_START_BPS.saturating_sub(steps * ZONE_B_STEP_BPS)
            }
        }
        ...
        /// Fire-and-forget XCC to register `participant` with the Sequencer
        /// Satellite for the current epoch.  Failures are swallowed — the
        /// buy itself must not revert if the Sequencer is full or unset.
        fn try_register_with_sequencer(&self, participant: Address) -> Result<(), Error> {
            use ink::env::call::{build_call, ExecutionInput, Selector};
            let satellite = self.sequencer_satellite.ok_or(Error::NoSatelliteRegistered)?;
            let _ = build_call::<ink::env::DefaultEnvironment>()
                .call(satellite)
                .exec_input(
                    ExecutionInput::new(Selector::new(
                        ink::selector_bytes!("register_participant"),
                    ))
        ...
        /// Check whether an address is whitelisted for pre-trading transfers.
        ///
        /// Whitelisted: owner, Dampener, DEX router.
        /// Used to gate transfer() and transfer_from() before trading_enabled.
        fn is_transfer_whitelisted(&self, addr: Address) -> bool {
            if addr == self.owner {
                return true;
            }
            if self.transfer_whitelist_dampener {
                if let Some(dampener) = self.dampener_address {
                    if addr == dampener { return true; }
                }
            }
            if let Some(router) = self.transfer_whitelist_dex_router {
                if addr == router { return true; }
            }
            false
        }
        ...
        fn assert_above_threshold(&self, amount: U256) -> Result<(), Error> {
            let elapsed = self.env().block_number().saturating_sub(self.deploy_block);
            let min = if elapsed < ZONE_A_END_BLOCK {
                SNIPER_MIN_THRESHOLD       // Zone A: 1 000 QF minimum
            } else {
                MIN_TRANSACTION_THRESHOLD  // Zone B onwards: 1 QF minimum
            };
            if amount < U256::from(min) {
    ...
    mod tests {
        ...
        fn deploy() -> Project52F {
            ...
            Project52F::new(U256::from(SUPPLY), "Project 52F".into(), "QF".into(), U256::ZERO)
        }
        ...
        // ── Sniper Harvest constants [*] ──────────────────────────────────────

        #[ink::test]
        fn sniper_constants_zone_boundary() {
            assert_eq!(ZONE_A_END_BLOCK, SNIPER_END_BLOCK / 2,
                "Zone A ends exactly halfway through the harvest window");
            assert_eq!(ZONE_B_START_BPS, 5_200, "Zone B opens at 52%");
            assert_eq!(ZONE_A_TAX_BPS,   9_500, "Zone A is flat 95%");
        }

        #[ink::test]
        fn sniper_zone_b_step_count_is_50() {
            // (10_400 - 5_200) / 104 = 50 steps → floor rate = 52% - 50% = 2%?
            // No: step 49 is block 10_296 (still in window), step 50 is block 10_400
            // which is already Hardening. So last step in window is step 49 → 3%.
            let steps_in_window = (SNIPER_END_BLOCK - ZONE_A_END_BLOCK) / ZONE_B_STEP_BLOCKS;
            assert_eq!(steps_in_window, 50);
            // Floor rate inside window (step 49):
            let floor = ZONE_B_START_BPS - 49 * ZONE_B_STEP_BPS;
            assert_eq!(floor, 300, "Zone B floor inside window is 3% (300 BPS)");
        }

        // ── Math assertion: £1 000 snipe → 95 000 QF to Dampener [*] ─────────

        #[ink::test]
        fn sniper_math_1000_gbp_snipe_yields_95000_qf() {
            // £1 000 at £0.01/QF → 100 000 QF transacted
            // Zone A tax 95% → 95 000 QF to Dampener Vault
            let snipe_qf:    u128 = 100_000 * ONE_QF;
            let expected_qf: u128 =  95_000 * ONE_QF;
            let tax = snipe_qf * ZONE_A_TAX_BPS / BPS_DENOMINATOR;
            assert_eq!(tax, expected_qf,
                "£1 000 snipe must donate exactly 95 000 QF to Dampener Vault");
        }

        // ── Sniper Harvest — get_tax_rates view [*] ───────────────────────────

        #[ink::test]
        fn get_tax_rates_zone_a() {
            let e = deploy(); // deploy_block = 0
            let (buy, sell, damp) = e.get_tax_rates(0);
            assert_eq!(buy,  9_500, "Zone A buy = 95%");
            assert_eq!(sell, 9_500, "Zone A sell = 95%");
            assert_eq!(damp, 9_500, "Zone A: 100% to dampener");
        }

        #[ink::test]
        fn get_tax_rates_zone_a_last_block() {
            let e = deploy();
            let (buy, _, damp) = e.get_tax_rates(ZONE_A_END_BLOCK - 1);
            assert_eq!(buy, 9_500);
            assert_eq!(damp, 9_500);
        }

        #[ink::test]
        fn get_tax_rates_zone_b_start() {
            let e = deploy();
            // Block 5200 = step 0 → 52% (5200 BPS)
            let (buy, sell, damp) = e.get_tax_rates(ZONE_A_END_BLOCK);
            assert_eq!(buy,  5_200, "Zone B step 0 = 52%");
            assert_eq!(sell, 5_200);
            assert_eq!(damp, 5_200, "Zone B: 100% to dampener");
        }

        #[ink::test]
        fn get_tax_rates_zone_b_step_5() {
            let e = deploy();
            // Block 5200 + 5×104 = 5720 → step 5 → 52% - 5% = 47%
            let (buy, _, _) = e.get_tax_rates(ZONE_A_END_BLOCK + 5 * ZONE_B_STEP_BLOCKS);
            assert_eq!(buy, 4_700, "Zone B step 5 = 47%");
        }

        #[ink::test]
        fn get_tax_rates_zone_b_floor() {
            let e = deploy();
            // Block 10_296 = step 49 → 52% - 49% = 3%
            let (buy, _, damp) = e.get_tax_rates(ZONE_A_END_BLOCK + 49 * ZONE_B_STEP_BLOCKS);
            assert_eq!(buy,  300, "Zone B floor = 3%");
            assert_eq!(damp, 300, "floor: still 100% to dampener");
        }

        #[ink::test]
        fn get_tax_rates_hardening() {
            let e = deploy();
            let (buy, sell, damp) = e.get_tax_rates(SNIPER_END_BLOCK);
            assert_eq!(buy,  E_BUY_TAX_BPS  as u32);
            assert_eq!(sell, PI_SELL_TAX_BPS as u32);
            assert_eq!(damp, TAX_DAMPENER_SHIELD_BPS as u32);
        }

        #[ink::test]
        fn get_tax_rates_scarcity() {
            let e = deploy();
            let (buy, sell, damp) = e.get_tax_rates(SHIELD_END_BLOCK);
            assert_eq!(buy,  E_BUY_TAX_BPS  as u32);
            assert_eq!(sell, PI_SELL_TAX_BPS as u32);
            assert_eq!(damp, TAX_DAMPENER_BASE_BPS as u32);
        }

        // ── Sniper Harvest — live buy/sell routing [*] ────────────────────────

        #[ink::test]
        fn buy_zone_a_all_tax_to_dampener() {
            // 100% of 95% tax must land in dampener; prize and team stay zero.
            let mut e = deploy();
            set_caller(accounts().alice);

            e.buy(U256::from(ONE_QF * 100_000)).unwrap();
            let expected = U256::from(95_000_u128 * ONE_QF);
            assert_eq!(e.get_dampener_accumulated(), expected,
                "Zone A buy: 95 000 QF to dampener");
            assert_eq!(e.get_prize_pot(),        U256::ZERO, "prize = 0 in sniper window");
            assert_eq!(e.get_team_accumulated(), U256::ZERO, "team = 0 in sniper window");
        }

        #[ink::test]
        fn sell_zone_a_all_tax_to_dampener() {
            let mut e = deploy();
            set_caller(accounts().alice);

            e.sell(U256::from(ONE_QF * 100_000)).unwrap();
            assert_eq!(e.get_dampener_accumulated(), U256::from(95_000_u128 * ONE_QF));
            assert_eq!(e.get_prize_pot(),        U256::ZERO);
            assert_eq!(e.get_team_accumulated(), U256::ZERO);
        }

        #[ink::test]
        fn buy_zone_b_step_10_rate_correct() {
            // Block 5200 + 10×104 = 6240 → step 10 → 42%
            let mut e = deploy();
            set_block(ZONE_A_END_BLOCK + 10 * ZONE_B_STEP_BLOCKS);
            set_caller(accounts().alice);

            let amount = U256::from(ONE_QF * 10_000);
            e.buy(amount).unwrap();
            // 42% of 10_000 = 4_200 QF
            assert_eq!(e.get_dampener_accumulated(), U256::from(4_200_u128 * ONE_QF),
                "Zone B step 10: 42% to dampener");
            assert_eq!(e.get_prize_pot(), U256::ZERO);
        }

        #[ink::test]
        fn sniper_ends_at_block_10400() {
            // Block SNIPER_END_BLOCK must use hardening routing, not sniper.
            let mut e = deploy();
            set_block(SNIPER_END_BLOCK);
            set_caller(accounts().alice);

            e.buy(U256::from(ONE_QF * 10_000)).unwrap();
            // Hardening: dampener = 125 BPS = 125 QF on 10 000 QF
            assert_eq!(e.get_dampener_accumulated(), U256::from(1_250_u128 * ONE_QF),
                "at SNIPER_END_BLOCK, hardening routing resumes");
            assert!(e.get_prize_pot() > U256::ZERO, "prize pot must be non-zero in hardening");
        }
        ...
        fn drain_hardening_populates_lp_reserve() {
            ...
            let mut e = deploy();
            set_caller(accounts().alice);
            // Drain threshold is now a permanent constant (DEFAULT_DRAIN_THRESHOLD).
            // Use a massive buy to accumulate prize pot to threshold.
            // prize_bps=147, so need ~352M QF buy to accumulate 520M 52F at 1:1 price.
            // For testing we use an extremely large buy amount.
            let pre_supply = e.total_supply();
            e.buy(U256::from(ONE_QF) * U256::from(400_000_000u64)).unwrap();
        ...
        fn claim_lp_pair_tokens_decrements_reserve() {
            let mut e = deploy();
            set_caller(accounts().alice);
            // drain threshold is now a permanent constant
            e.buy(U256::from(ONE_QF * 10_000)).unwrap();
        ...
        fn request_great_drain_fires_hardening() {
            ...
            // prize_drain_threshold is now a compile-time constant — no setter needed.
            e.dampener_address = Some(accs.bob);
            set_caller(accs.bob);
        ...
        fn epoch_resets_at_52_buys() {
            ...
            // get_epoch_id removed — epoch id tracked by Sequencer Satellite
            e.buy(amt).unwrap();
